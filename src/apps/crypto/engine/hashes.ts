/**
 * Pure TypeScript SHA-256 cryptographic hashing and avalanche effect analysis
 */
...
function utf8Encode(str: string): Uint8Array {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    if (charCode < 0x80) {
      codeUnits.push(charCode);
    } else if (charCode < 0x800) {
      codeUnits.push(0xc0 | (charCode >> 6), 0x80 | (charCode & 0x3f));
...
export interface AvalancheResult {
  ...
  bitDiff: number;
  totalBits: number;
  bitDiffPercentage: number;
  bitMap: boolean[]; // true if bit flipped, false if identical
}

export function computeAvalanche(text1: string, text2: string): AvalancheResult {
  const hash1 = computeSha256(text1);
  const hash2 = computeSha256(text2);
  ...
  const totalBits = 256;
  const bitDiffPercentage = (bitDiff / totalBits) * 100;
  ...
  return {
    ...
    bitDiff,
    totalBits,
    bitDiffPercentage: Math.round(bitDiffPercentage * 100) / 100,
    bitMap
  };
}
